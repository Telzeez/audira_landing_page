...
export function createUser(user: Omit<UserRecord, 'id' | 'points' | 'registeredDevices'>): UserRecord | null {
  const users = getUsers();
...
export function updateUser(id: string, updates: Partial<Omit<UserRecord, 'id' | 'email'>>): UserRecord | null {
  const users = getUsers();
