...
export async function POST(request: Request) {
  try {
    ...
    if (!role || typeof role !== 'string' || !role.trim()) {
      return NextResponse.json({ error: 'Profession or role is required.' }, { status: 400 });
    }
    if (role.trim().length > 50) {
      return NextResponse.json({ error: 'Profession/Role must be less than 50 characters.' }, { status: 400 });
    }
    ...
    if (quote.trim().length > 500) {
      return NextResponse.json({ error: 'Review text must be less than 500 characters.' }, { status: 400 });
    }
    ...
    if (hasReviewed) {
      return NextResponse.json({ error: 'You have already submitted a review. Thank you for your feedback!' }, { status: 400 });
    }

    // 5. Alternate avatars for variety
    const imageCount = reviews.length;
    const avatarImage = imageCount % 2 === 0 ? '/images/user_profile_1.png' : '/images/user_profile_2.png';
