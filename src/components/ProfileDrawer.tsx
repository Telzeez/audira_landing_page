...
export default function ProfileDrawer({
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    const endpoint =
      authView === "signin" ? "/api/auth/login" : "/api/auth/signup";
    const payload =
      authView === "signin" ? { email, password } : { name, email, password };
    ...
    try {
      ...
      if (!response.ok) {
        setAuthStatus("error");
        setAuthMessage(
          data.error || "Authentication failed. Please check inputs.",
        );
        return;
      }
      ...
      setAuthMessage(
        authView === "signin" ? "Successfully logged in." : "Account created!",
      );
  ...
  const handleForgotSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setResetMessage(data.error || "Unable to send reset instructions.");
        return;
      }
      ...
      setResetMessage(
        data.resetToken
          ? "Reset code generated. Enter your code and a new password below."
    ...
    <>
      ...
      <div className={`${styles.drawer} ${isOpen ? styles.drawerActive : ""}`}>
        ...
        <div className={styles.body}>
          {!user ? (
            /* Logged Out: Sign In / Sign Up / Forgot Password Forms */
            <div
            ...
            >
              ...
              {(authView === "signin" || authView === "signup") && (
                <form className={styles.form} onSubmit={handleAuthSubmit}>
                  ...
                  {authStatus !== "idle" && authStatus !== "loading" && (
                    <div
                      className={`${styles.formMessage} ${authStatus === "success" ? styles.messageSuccess : styles.messageError}`}
                    >
                      {authMessage}
                    </div>
                  )}
              ...
              {authView === "forgot" && (
                <form className={styles.form} onSubmit={handleForgotSubmit}>
                  ...
                  {resetStatus !== "idle" && resetStatus !== "loading" && (
                    <div
                      className={`${styles.formMessage} ${resetStatus === "success" ? styles.messageSuccess : styles.messageError}`}
                    >
                      {resetMessage}
                    </div>
                  )}
              ...
              <div>
                ...
                <form className={styles.form} onSubmit={handleRegisterDevice}>
                  ...
                  {regStatus !== "idle" && regStatus !== "loading" && (
                    <div
                      className={`${styles.formMessage} ${regStatus === "success" ? styles.messageSuccess : styles.messageError}`}
                    >
                      {regMessage}
                    </div>
                  )}
                    ...
                    registeredDevices.map((d, index) => (
                      <div key={index} className={styles.deviceItem}>
                        ...
                        <div className={styles.deviceDetails}>
                          ...
                          <span className={styles.deviceSerial}>
                            {d.serial}
                          </span>
                        </div>
                      </div>
